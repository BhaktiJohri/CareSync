...
/**
 * Generates daily dose events from medications
 * @param {import('./types').Medication[]} medications
 * @returns {import('./types').DoseEvent[]}
 */
export const generateDailyDoses = (medications) => {
    const doses = [];
...
/**
 * Check for dose reminders
 * @param {import('./types').DoseEvent[]} doses
 * @returns {import('./types').DoseEvent[]}
 */
export const checkReminders = (doses) => {
    const now = new Date();
 ...
 * @param {string} type
 * @param {string} value
 * @returns {import('./types').VitalStatus}
 */
export const analyzeVitalSign = (type, value) => {
